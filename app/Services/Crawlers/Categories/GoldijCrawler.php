...
class GoldijCrawler extends AbstractCategoriesCrawler
{
    /**
     * @override
     */
        ...
        try {
            ...
            $wait->until(
                WebDriverExpectedCondition::presenceOfElementLocated(
                    WebDriverBy::xpath("/html/body//header[1]/div[2]/div[1]/nav/ul")
                )
            );

            $shopLink = $elementExtractor->getElementByXPath($driver, '/html/body//header[1]/div[2]/div[1]/nav/ul/li[1]/a')->getAttribute('href');
            $driver->get($shopLink);
            $wait->until(
                WebDriverExpectedCondition::presenceOfElementLocated(
                    WebDriverBy::xpath("//ul[contains(@class, 'product-categories')]")
                )
            );

            $categoryWrapper = $driver->findElement(WebDriverBy::xpath("//ul[contains(@class, 'product-categories')]"));
